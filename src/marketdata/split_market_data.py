...
def get_ohlc_for_column(df: pd.DataFrame, col_name: str, date_col_name: str) -> pd.DataFrame:
    if df.dropna().empty:
...
def split_column_to_ohlc(df: pd.DataFrame, data_col_name:str = "date") -> List[ResultSet]:
    results = [ResultSet(column, get_ohlc_for_column(df, column, data_col_name)) for column in df.columns if column != data_col_name]
    return results
