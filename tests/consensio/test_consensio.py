...
def test_return_mean_serie():
    ...
    result_data = [
        ...
        {'date': '2018-1-2', 'close': 1, 'roll_mean': 1.},
        {'date': '2018-1-3', 'close': 1, 'roll_mean': 1.},
        {'date': '2018-1-4', 'close': 1, 'roll_mean': 1.},
        {'date': '2018-1-5', 'close': 1, 'roll_mean': 1.},
        {'date': '2018-1-6', 'close': 1, 'roll_mean': 1.},
        {'date': '2018-1-7', 'close': 1, 'roll_mean': 1.},
        {'date': '2018-1-8', 'close': 1, 'roll_mean': 1.},
        {'date': '2018-1-9', 'close': 1, 'roll_mean': 1.},
    ]
    ...
    pd.testing.assert_frame_equal(df_result, df, check_less_precise=True, check_like=True)
...
def test_calculate_price_increase_for_column():
    ...
    pd.testing.assert_frame_equal(df_result[['date', 'close_up']], df[['date', 'close_up']], check_less_precise=True,
                                  check_like=True)
...
def test_return_weight_if_col1_greater_than_col2():
    ...
    result_data = [
        ...
        {'date': '2018-1-2', 'close': 2, 'short_MA': 1.5, 'close<short_MA': 2.},
        {'date': '2018-1-3', 'close': 3, 'short_MA': 2.5, 'close<short_MA': 2.},
        {'date': '2018-1-4', 'close': 4, 'short_MA': 3.5, 'close<short_MA': 2.},
        ...
        {'date': '2018-1-8', 'close': 1, 'short_MA': 1, 'close<short_MA': 2.},
        {'date': '2018-1-9', 'close': 0, 'short_MA': 0.5, 'close<short_MA': 0},
    ]

    df = pd.DataFrame(input_data)
    df['close<short_MA'] = get_value_between_two_col(df, 'close', 'short_MA', 2., 0)
    df_result = pd.DataFrame(result_data)
    pd.testing.assert_frame_equal(df_result[['date', 'close<short_MA']], df[['date', 'close<short_MA']],
...
def test_consensio(source_path: str, result_file_path: str, comment: str):
    ...
    df_source = Loader.get_historical_data(source_path, Source.LocalData, time_frame=TimeFrame.Weekly)
    ...
    df_consensio.sort_values('date', ascending=True).to_csv(result_output_path, header=True, index=False)

    if not os.path.exists(result_file_path):
        print("Result file doesnt exist")
        return

    df_result = pd.read_csv(result_file_path, header=0, parse_dates=['date'], infer_datetime_format=True)

    df_result = df_result.reset_index(drop=True).sort_values('date', ascending=True).reset_index()
    df_consensio = df_consensio.reset_index(drop=True).sort_values('date', ascending=True).reset_index()
    pd.testing.assert_frame_equal( \
        df_result.sort_values('date', ascending=True)[['date', 'consensio']],
        df_consensio.sort_values('date', ascending=True)[['date', 'consensio']], check_less_precise=True, check_like=True)
