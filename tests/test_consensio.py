...
def test_return_mean_serie():
    ...
    result_data = [
        ...
        {'dateu': '2018-1-2', 'close': 1, 'roll_mean': 1.},
...
def get_test_sample_data():
    sample_data_folder = os.environ.get("HW_DATA_ROOT_FOLDER", "./consensio")
...
def test_hyperwaves(source_name: str, result_file_name: str,  comment: str):
    ...
    consensio = get_consonsio(df_source, "close", low_ma=3, mid_ma=7, high_ma=30 )
    sample_data_folder = os.environ.get("HW_DATA_ROOT_FOLDER", "./consensio")
    ...
    consensio.sort_value('date', ascending=True).to_csv(result_output_path , header=True, index=False)
    ...
    df_result = df_result.sort_values('date', ascending=True).reset_index()
    comsensio = consensio.sort_values('date', ascending=True).reset_index()
    pd.testing.assert_frame_equal(df_result, comsensio, check_less_precise=True, check_like=True )
