...
def get_test_sample_data():
    ...
    for file_name in os.listdir(sample_data_folder):
        ...
        if '.hw' in file_name or '.hull' in file_name:
            continue
        yield (os.path.join(sample_data_folder, file_name), os.path.join(sample_data_folder, file_name_json_result),
               "test file {}".format(file_name))


# @pytest.mark.usefixtures("set_env_variable_hyperwave")
@pytest.mark.parametrize("source_name, result_file_path, comment", get_test_sample_data())
def test_hyperwaves(source_name: str, result_file_path: str, comment: str):
    df_source = Loader.get_historical_data(source_name, Source.LocalData, time_frame=TimeFrame.Weekly)
